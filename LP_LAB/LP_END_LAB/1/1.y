...
%left AND
%token <d> op
%token <d> VARIABLE INT WHILE FOR IF THEN ELSE BOOL FLOAT AND OR NOT
SWITCH CASE BREAK ob cb DO BOOL_VAL FLOAT_VAL INT_VAL
%type <d> program statement expr newline decl type vals decls cases /*obrace cbrace*/
%%
program:
decls program statement '#' newline{ //complete set of code is returnedhere
//'#' denotes end of code
...
| {}
;
type :INT { string code1="int"; //for printing types in declaration
$$.code=new char[code1.length()]; //part
...
;
decl : type vals ';' { /*type and variables return here*/
string code1="";
string ty="";
ty.append($1.code);
code1.append($1.code);
...
;
vals: expr { /*for last variable in
declaration*/
...
|expr '=' expr ',' vals{ /*for variables which are initialized*/
if($1.EXTRA.var_flag==2||$1.EXTRA.var_flag==3)
{cout<<"error in declaration :("<<endl;exit(0);}
string code1="";
vars.push_back($1.place);
...
|expr ',' vals { /*if variables are not yet
done*/
if($1.EXTRA.var_flag==2||$1.EXTRA.var_flag==3)
{cout<<"error in declaration:("<<endl;exit(0);}
string code1="";
...
| { }
;
statement: /*for different statements in the
code*/
...
| IF '(' expr ')' '{' newline statement '}' newline ELSE '{' newline statement '}' newline
statement {
/*IF ELSE */
...
| FOR '(' expr ';' expr ';' expr ')' '{' newline statement '}' newline statement{
/*FOR*/
...
| DO '{' newline statement '}' WHILE '(' expr ')' ';' newline statement {
string begin="l"+to_string(lc++);
string after="l"+to_string(lc++);
string code1="";
code1.append(begin);
...
int
x=check_compatibility($1.place,$3.place,$1.EXTRA.var_flag,$3.EXTRA.var_flag);
string code1="";
string place1="";
...
| expr '=' expr '?' expr ':' expr ';' newline statement {
string code1="";
string sl1="l"+to_string(lc++);
string sl2="l"+to_string(lc++);
code1.append($3.code);
code1+=gen_code("if ",$3.place," = 0 "," goto ");
code1.append(sl1);
code1.append("\n");
code1.append($1.place);
code1.append(" = ");
code1.append($5.place);
code1.append("; \ngoto ");
code1.append(sl2);
code1.append("\n");
code1.append(sl1);
...
| {string code1="";$$.code=new
char[code1.length()];strcpy($$.code,code1.c_str());}
;
expr: expr '+' expr {
/*for ADDITION*/
$$.EXTRA.var_flag=2;
...
string place1="";
int
x=check_declaration($1.place,$3.place,$1.EXTRA.var_flag,$3.EXTRA.var_flag);
...
|INT_VAL { $$.EXTRA.var_flag=3; //var_flag is to differentite integerand
//float from others
string code1;
string place1;
code1.append("");
