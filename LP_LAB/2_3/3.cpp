...
vector<vector<pair<char, pair<string, char> > > > itemsets,itemsets1;
map<pair<int, char>, int> goto1,goto2;
...
void build_items(char sa)
{
	...
	itemsets1.push_back(closure(init)); 
	...
	while(z<s.size())
	{
		...
		z++;
		int index = 0;
		for(int i=0; i<itemsets1.size(); i++)
		{
			if(now==itemsets1[i])
			{
				index=i; 
		...
		for(int it=0; it<u; it++)
		{
			...
			for(int i=0; i<itemsets1.size(); i++)
			{
				if(itemsets1[i]==curr_close)
				{
					goto2[make_pair(index, k1[it])] = i;
					flag = 1; 
					break;
				}
			}
			if(flag==0)
			{
				goto2[make_pair(index, k1[it])] = itemsets1.size();
				itemsets1.push_back(curr_close);
...
void merge_items()
{
	...
	for(int i=0; i<itemsets1.size(); i++)
	{
		for(int j=0; j<i; j++)
		{
			...
			for(int k=0; k<itemsets1[i].size(); k++)
			its1.insert(make_pair(itemsets1[i][k].first, itemsets1[i][k].second.first));
			for(int k=0; k<itemsets1[j].size(); k++)
			its2.insert(make_pair(itemsets1[j][k].first, itemsets1[j][k].second.first));
			if(its1==its2)
			{
				present[i]=false;
				int p=-1,q,k;
	...
	for(int k=0;k<merge.size();k++)
	{
		for(int l=0;l<merge[k].size();l++)
		{
			...
			if(l==0)
			{
				itemsets.push_back(itemsets1[merge[k][l]]);
			}
			else
			{
				for(int i=0; i<itemsets1[merge[k][l]].size(); i++)
				{
					if(find(itemsets[k].begin(),itemsets[k].end(),itemsets1[merge[k][l]][i])==itemsets[k].end())
					itemsets[k].push_back(itemsets1[merge[k][l]][i]);
				}
			}
		}
		cout<<"\n";
	...
	for(it1=goto2.begin(); it1!=goto2.end(); it1++)
	{
		int p,q;
		for(int k=0;k<merge.size();k++)
		{
...
//					cout<<"GOTO["<<(it1->first).first<<", "<<(it1->first).second<<"] = "<<it1->second<<endl;
		...
		goto1[make_pair(p, (it1->first).second)] = q;
	}
